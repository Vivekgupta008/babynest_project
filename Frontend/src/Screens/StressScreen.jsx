...
export default function StressScreen() {
  ...
  const [level, setLevel] = useState(''); // e.g. 1-10 or Low/Medium/High
  ...
  const fetchLogs = async () => {
    const res = await fetch(`${BASE_URL}/get_stress_logs`);
  ...
  const handleSubmit = async () => {
    await fetch(`${BASE_URL}/set_stress_log`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({week_number: week, level, trigger, note}),
    });
    setWeek('');
    setLevel('');
    setTrigger('');
  ...
  return (
    <View style={styles.container}>
      <HeaderWithBack title="Stress Level Tracker" />
      <ScrollView
        ...
        <Card style={styles.formCard}>
          <Card.Content>
            <Text style={styles.sectionTitle}>Log Stress Entry</Text>
            <TextInput
              label="Week Number"
              value={week}
            ...
            />
            <TextInput
              label="Stress Level (1-10 or Low/Med/High)"
              value={level}
          ...
          <Card key={index} style={styles.entryCard}>
            <Card.Content>
              <Text style={styles.entryText}>
                Week {entry.week_number} - Level: {entry.level}
              </Text>
              {entry.trigger ? <Text>Trigger: {entry.trigger}</Text> : null}
              {entry.note ? (
